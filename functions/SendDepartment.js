...
const dep = async function deps(collage){
    return new Promise(function (resolve, reject) {
        con.query("SELECT D.*,c.name FROM dep D, colleges c WHERE D.idcol=? AND c.id=?",[collage,collage], (err, result) => {
            ...
            let list = new List(`${collage_name} department`, `${collage_name}`, sections, 'Departments', '');
