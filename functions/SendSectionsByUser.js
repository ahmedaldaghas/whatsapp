  ...
  return new Promise(function (resolve, reject) {
    ...
    con.query(
      ...
      (err, result) => {
        ...
        con.query(
          "SELECT sections.*, subjects.subject, links.link FROM ((sections LEFT JOIN links ON links.idsec = sections.id) INNER JOIN subjects ON subjects.id = (SELECT id FROM subjects WHERE subject=?)) WHERE sections.idsc=(SELECT id FROM subjects WHERE subject=?) ORDER BY sections.sec;",
          [subject, x],
          (err, result) => {
            if (err) return reject(err);
            if (result.length === 0)
              return resolve(
                "Are you sure this subjects exists ^0^ Please send a number from 1-9 to see the list"
              );
