...
const sections = async function section(subject) {
    return new Promise(async function (resolve, reject) {
        let x = subject;
        con.query("SELECT link FROM subjects WHERE id=? AND link!='NULL';", [subject], (err, result) => {
            if(err) return reject(err);
            if (result.length === 1) x = result[0].link;
            con.query("SELECT sections.*, subjects.subject, links.link FROM ((sections LEFT JOIN links ON links.idsec = sections.id) INNER JOIN subjects ON subjects.id = ?) WHERE sections.idsc=? ORDER BY sections.sec;", [subject, x], (err, result) => {
                if (err) return reject(err);
                if (result.length === 0) return resolve("Sorry we don't have this course available");
