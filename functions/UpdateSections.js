                                        ...
                                        (err, result) => {
                                            if (err) {
                                                ...
                                                $(".large-12.medium-6.columns").map(function () {
                                                    ...
                                                    a.sec.forEach((c,i) => {
                                                        if(i === 1){
                                                            b += `((SELECT id from subjects WHERE subject="${a.subject}"),${0}),`
                                                        }
                                                        b += `((SELECT id from subjects WHERE subject="${a.subject}"),${c}),`
                                                    })
                                                })

                                                con.query(
                                                    "INSERT INTO sections(idsc,sec) VALUES " + b.slice(0, -1)  + " ON DUPLICATE KEY UPDATE id=id",
                                                    (err, result) => {
                                                        if (err) {
                                                            con.query("SET FOREIGN_KEY_CHECKS=1;")
                                                            return resolve(
